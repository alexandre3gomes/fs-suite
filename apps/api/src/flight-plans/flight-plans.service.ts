...
function mapPlanFields(dto: Partial<CreateFlightPlanDto>): Partial<Omit<Prisma.FlightPlanCreateInput, 'user' | 'routes' | 'visualReferences' | 'briefingItems'>> {
  return {
    ...
    destinationTpaFt: dto.destinationTpaFt,
    destinationTpaSource: dto.destinationTpaSource,
    altitudeChanges: dto.altitudeChanges != null ? JSON.parse(JSON.stringify(dto.altitudeChanges)) : undefined,
    ...
    const plan = await this.prisma.$transaction(async (tx) => {
      return tx.flightPlan.create({
        data: {
          ...
          destinationTpaFt: original.destinationTpaFt,
          destinationTpaSource: original.destinationTpaSource,
          altitudeChanges: original.altitudeChanges ?? undefined,
