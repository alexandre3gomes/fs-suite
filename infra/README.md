...
API runs on an OVHcloud VPS in Frankfurt (the only paid resource, EUR 5.52/mo
incl. VAT; everything else is free-tier).
Build once on GitHub Actions, deploy to the VPS.
...
│   └── setup.sh             # File-driven one-time host provisioning
└── README.md
```
...
| API | OVHcloud VPS-1 | Production | Ubuntu 26.04, Frankfurt (DE) — same city as the Supabase primary |
...
|----------|---------|-------|
| GHCR | VPS | `ghcr.io/alexandre3gomes/fs-suite-api` |

### Secrets Management

| Runtime | Where | How |
|---------|-------|-----|
| VPS | `/opt/fs-suite/.env` | File on disk, `chmod 600` |
...
| `api.fs-suite.com` | A | OVH VPS IPv4 (`162.19.243.126`) | Proxied |
...
```bash
# 1. Order an OVHcloud VPS-1 (Ubuntu 26.04) in Frankfurt
# 2. Note the VPS IPv4 (fixed for the life of the instance)
...
The host uses a **Cloudflare Origin Certificate** for TLS on the origin:
...
manager) and is propagated to the host by `infra/vps/setup.sh`.

**First-time provisioning** (`JWT_PRIVATE_KEY` / `JWT_PUBLIC_KEY` blank
in canonical `.env`):

1. Run `infra/vps/setup.sh` — it detects the absence and generates a
...
populated in canonical `.env`): host setup detects the existing values
...
was leaked): clear both fields in the canonical `.env`, run host setup
...
- Authentication is key-only; the host firewall is `ufw` (a plain VPS has no
  security-group layer in front of the machine, unlike the previous EC2)
- Root login is disabled (`PermitRootLogin no` in AMI default)
- Only `ubuntu` is accessible

The setup script does not modify `sshd_config`. These protections depend on the Ubuntu
...
- The SSH key is stored in GitHub Secrets (`DEPLOY_SSH_KEY`) and in a password manager
...
└────┬────┘
     ...
     ▼
┌──────────┐
│ Migrate  │  prisma migrate deploy (via SSH to the host)
...
**Rollout order**: build → migrate → deploy host. Migrations run against
...
| `deploy.yml` | Push to `main` (API/packages paths) | Build → migrate → deploy host |
...
- **Automated deploys**: `GITHUB_TOKEN` from Actions, passed via SSH to the host
...
|---|----------|----------|------------------|---------------|
| A | API runtime | NestJS process | Yes | VPS `/opt/fs-suite/.env` (`infra/vps/setup.sh`) |
...
| `DEPLOY_HOST` / `DEPLOY_SSH_KEY` / `DEPLOY_USER` | C | — | — | ✅ (`deploy.yml`) |
...
keys are blank, `infra/vps/setup.sh` generates a fresh RS256 pair on
...
subsequent reprovisions with the values already in `.env`, the host picks
...
```bash
# 1. Host — propagates every API-runtime secret from .env automatically
scp .env origin.pem origin-key.pem fs-suite:~/
...
changes. See the SSH key step in [Host setup](#host-setup).
...
| `JWT_PRIVATE_KEY` / `JWT_PUBLIC_KEY` | RS256 keypair. Lives in the canonical `.env`. Generated by `infra/vps/setup.sh` on first provision only; subsequent reprovisions reuse the values from `.env` (sessions stay valid). To force a rotation, clear both fields in `.env` before running setup. See [JWT Keys](#jwt-keys). |
...
| `OWM_API_KEY` | OpenWeatherMap key — precipitation tile proxy. Optional; if you start using it, add it to the host `.env`. |
...
| `DEPLOY_HOST` / `DEPLOY_SSH_KEY` / `DEPLOY_USER` | SSH access to the deploy target. |
...
your `.env` (category A + D) and is replicated to the host and GitHub Secrets
...
|---|---|---|---|
| **Host disk** | `df -h /` on the API host: % used, total size, count of docker images (`docker images --filter dangling=false`). | SSH to the host from the workflow runner. | Investigate at >85% — extraction may start failing. |
...
`fs-suite-api` images** on the API host before every pull (current
...
The daily Metrics Digest also reports `Host disk: X% of YG · N docker
...
- **Do NOT** run `docker logout` followed by a re-login with a different
  account on the host — the persistent PAT credentials from `setup.sh`
...
> Cloudflare Worker were decommissioned to keep cost at zero (the VPS is the
> only paid resource). The VPS is the sole API runtime. If a managed-runtime
> path is wanted later (e.g. to drop the always-on VPS cost), it can be
> re-provisioned
> from scratch — but resolve the Cloud Run cold-start startup-probe behaviour
> first.

### What changes on host reprovisioning
...
|----------|----------|-----------|
| JWT keypair lives in canonical `.env` | Operator must capture the pair into `.env` after the first-ever host setup | Single source of truth; reprovisions preserve sessions; intentional rotation is a single-line edit |
...
| The VPS is the only API runtime | No instant managed-runtime failover | Keeps cost at EUR 5.52/mo and nothing else; the Cloud Run candidate was decommissioned (2026-06) and the EC2 it replaced belonged to a project that went away (2026-09) |
